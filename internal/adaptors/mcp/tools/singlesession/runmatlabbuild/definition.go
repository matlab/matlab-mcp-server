...
const (
	...
	description = "Run a MATLAB Build Tool build (`buildtool`) in an existing MATLAB session. Executes one or more build tasks defined in a `buildfile.m`. Optionally specify a working directory containing the `buildfile.m`, specific task names to run, and build options. Returns the full build log and whether the build succeeded."
)
